...
struct PCB{
    ...
    int             priority;
    unsigned int    io_saved;
    unsigned int    holder;

bool operator==(const PCB& other){
return (PID == other.PID && state == other.state && partition_number == other.partition_number && size == other.size);
}};
...
PCB add_process(std::vector<std::string> tokens) {
    ...
    process.priority = 0;
    process.io_saved = process.io_freq;
    process.holder = process.io_freq;
    return process;
}
